...
function ScoreBar({ value, animated }: { value: number; animated: boolean }) {
  const color =
    value >= 90 ? "#1A5C3A" : value >= 70 ? "#4A8A6A" : "#D97706";
...
export default function AuditScoreCard() {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(13,13,11,0.03)", border: "1px solid rgba(13,13,11,0.06)" }}
    >
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[10px] uppercase tracking-[0.15em] font-semibold" style={{ color: "#6B6B67" }}>
            GBP Audit Score
          </p>
          <div className="flex items-baseline gap-1.5 mt-1">
            <motion.span
              ...
              style={{ fontFamily: "var(--font-outfit)", color: phase === "after" ? "#1A5C3A" : "#D97706" }}
            >
              {overallScore}
            </motion.span>
        ...
        <div
          ...
          style={{
            background: phase === "after" ? "#E8F2EC" : "#FEF3C7",
            color: phase === "after" ? "#1A5C3A" : "#D97706",
          }}
        >
