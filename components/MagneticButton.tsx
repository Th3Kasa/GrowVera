...
export default function MagneticButton({
  children, className = "", style, onClick, href, strength = 0.3,
  as: Tag = "button", type = "button", disabled = false, ariaLabel,
}: MagneticButtonProps) {
  ...
  if (Tag === "a" && href) {
    return (
      <div ref={ref} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ display: "inline-block" }}>
        <motion.a href={href} className={className} style={{ ...style, ...motionStyle }} aria-label={ariaLabel} whileTap={{ scale: 0.97 }} transition={springTransition}>{children}</motion.a>
      </div>
    );
  }
  return (
    <div ref={ref} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ display: "inline-block" }}>
      <motion.button type={type} className={className} style={{ ...style, ...motionStyle }} onClick={onClick} disabled={disabled} aria-label={ariaLabel} whileTap={{ scale: 0.97 }} transition={springTransition}>{children}</motion.button>
    </div>
  );
}
